#ifndef RADIOIFY_AUDIOFILTER_MATH_SIGNAL_MATH_H
#define RADIOIFY_AUDIOFILTER_MATH_SIGNAL_MATH_H
...
inline constexpr float kSignalMathTwoPi = 6.28318530717958647692f;
inline constexpr float kSignalMathLinDbFloor = 1e-12f;
inline constexpr float kSignalMathHashUnitInv = 1.0f / 4294967295.0f;
inline constexpr float kSignalMathDefaultSoftClipThreshold = 0.98f;
...
inline float wrapPhase(float phase) {
  while (phase >= kSignalMathTwoPi) phase -= kSignalMathTwoPi;
  while (phase < 0.0f) phase += kSignalMathTwoPi;
  return phase;
}
...
inline float lin2db(float x) {
  return 20.0f * std::log10(std::max(x, kSignalMathLinDbFloor));
}
...
inline float seededSignedUnit(uint32_t seed, uint32_t salt) {
  ...
  return 2.0f * (static_cast<float>(h) * kSignalMathHashUnitInv) - 1.0f;
}
...
inline float softClip(float x,
                      float t = kSignalMathDefaultSoftClipThreshold) {
  float ax = std::fabs(x);
  if (ax <= t) return x;
...
#endif  // RADIOIFY_AUDIOFILTER_MATH_SIGNAL_MATH_H
