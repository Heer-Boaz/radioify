...
constexpr auto kMiniPlayerRefreshInterval = std::chrono::milliseconds(250);
...
WindowUiState buildPlaybackFramebufferUiState(
  ...
  overlayInputs.miniPlayerTuiAvailable = videoWindow.IsOpen();
  overlayInputs.miniPlayerTuiActive =
      overlayInputs.miniPlayerTuiAvailable &&
      videoWindow.IsMiniPlayerTui();
...
void runFramebufferPresenterLoop(
  ...
  while (threadState.load(std::memory_order_relaxed) !=
    ...
    const bool miniPlayerRequested = videoWindow.IsMiniPlayerTui();
    if (!forcePresentRequested) {
      int waitTimeoutMs = -1;
      auto tightenWaitTimeout = [&](int candidateMs) {
        if (candidateMs < 0) return;
        waitTimeoutMs =
            waitTimeoutMs < 0 ? candidateMs : std::min(waitTimeoutMs, candidateMs);
      };
      ...
      if (miniPlayerRequested) {
        ...
        if (lastMiniPlayerPresent ==
          ...
          tightenWaitTimeout(std::max(
              0, static_cast<int>(std::chrono::duration_cast<
                                       std::chrono::milliseconds>(
                                       (lastMiniPlayerPresent +
                                        kMiniPlayerRefreshInterval) -
                                       now)
                                       .count())));
        }
      }
