...
DemuxResult Controller::seekForPendingRequest(
  ...
  seekRequest.seekUs = playback_timeline::prerollSeekUs(result.targetUs);
  seekRequest.logTag = "audio_track_switch_seek";
  seekRequest.logPath = context.logPath;
  result.seek = playback_timeline::seekPrimaryDemux(seekRequest);

  videoPreroll_ = result.seek.seeked
                      ? playback_timeline::beginPrerollDiscard(result.targetUs)
                      : playback_timeline::PrerollDiscard{};
...
bool Controller::shouldDropVideoPrerollPacket(const AVPacket& packet,
  ...
  return playback_timeline::shouldDropPrerollPacket(
